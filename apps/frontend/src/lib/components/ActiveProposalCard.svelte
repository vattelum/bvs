<script lang="ts" module>
	import type { ProposalStatus as PS } from '$lib/services/snapshot-x';
		...
		<div class="mt-3">
			...
			<div class="flex items-center justify-between mt-2 text-xs">
				...
				<span class="text-text-muted">
					{#if totalSupply > 0n}
						Turnout: {Number(totalVotes)} of {Number(totalSupply)} members ({Math.round(Number(totalVotes * 100n) / Number(totalSupply))}%)
					{:else}
						Turnout: {Number(totalVotes)}
					{/if}
				</span>
			</div>
		</div>
