...
<div class="border border-border rounded-lg p-4">
	<label class="flex items-center gap-2 cursor-pointer">
		...
		<Tooltip text={"Restrictions protect the document from changes after recording.\n\nSection locks block amendments to chosen sections at the contract level (BVSRegistry.addDocument reverts SectionLocked).\n\nTime locks set a minimum wait between amendments (BVSRegistry.addDocument reverts AmendmentTooSoon)."} align="left"><span class="text-text-muted cursor-help text-xs">(?)</span></Tooltip>
	</label>
	...
	{#if enabled}
		<div class="mt-4 flex flex-col gap-4">
			...
			<div>
				<label class="block text-sm text-text-secondary mb-1">Locked sections <span class="text-text-muted">(amendments blocked on-chain)</span></label>
				...
				{#if lockMode === 'specific'}
					...
					{/if}
				{:else}
					<p class="text-xs text-text-muted">Every section is blocked from amendment for the configured duration.</p>
				{/if}
			</div>

			<!-- Duration (second) -->
			<div>
				<label class="block text-sm text-text-secondary mb-1">Duration <span class="text-text-muted">(minimum time between amendments)</span></label>
				...
				{#if timeLockPreset === 'permanent'}
					<p class="text-xs text-text-muted mt-1">Locked sections cannot be amended; only the time gate is dropped.</p>
				{:else if timeLockSeconds > 0}
					<p class="text-xs text-text-muted mt-1">After each amendment, the next one is blocked for {Math.round(timeLockSeconds / 86400)} days.</p>
				{/if}
			</div>
		</div>
	{/if}
