...
// Turbo's official upload service. Same URL the @ardrive/turbo-sdk
// uses internally (defaultUploadServiceURL); pinned here so
// verifyTurboHas and any future status calls share one source of
// truth.
const TURBO_UPLOAD_SERVICE = 'https://upload.ardrive.io';

// Confirm Turbo has the bytes for the given tx ID before we let the
// caller commit it on-chain. The SDK's upload() resolves on
// receipt-signed, which under rare conditions can precede the data
// landing in Turbo's system — and once an unreachable URI is in the
// registry the only fix is an on-chain amendment.
export async function verifyTurboHas(txId: string): Promise<void> {
	// Delay BEFORE each attempt: 0s, 3s, 8s, 15s. Turbo's status endpoint
	// often 404s for several seconds after upload() resolves while the
	// bundle propagates internally; tight retries just burn budget and
	// force the user through an M1→M2 resubmit on every upload.
	...
	throw new Error(
		'Turbo did not confirm the upload. Nothing was recorded on-chain. Please retry.'
	);
}
...
const CACHE_PREFIX = 'bvs:arweave:';
