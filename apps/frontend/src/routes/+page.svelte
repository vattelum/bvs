<script lang="ts">
	...
	import { loadCategories as fetchCategories, loadDocuments } from '$lib/services/registry';
	...
	interface DocEntry {
		...
		expanded: boolean;
		loading: boolean;
		/** Per-document amendment restrictions, present only when set. Drives
		 *  the on-row Locked / Restricted badge. Includes lastAmendmentTime so
		 *  the badge can compute when the time window expires. */
		restrictions?: {
	...
	// resolution suffices for day/hour-scale countdowns; cheaper than 1s.
		...
		try {
			...
			for (const ver of updatedVersions) {
				for (const ref of ver.references) {
					...
					if (
						ref.registryAddress.toLowerCase() !== bvsRegistryAddress.toLowerCase() ||
						ref.chainId !== chainId ||
	...
	function getRefDocTitle(ref: Reference): string {
		...
		if (ref.registryAddress.toLowerCase() !== bvsRegistryAddress.toLowerCase()) {
													...
													{#if doc.restrictions}
														...
														{@const sectionsLabel = isEntire
															...
															: r.lockedSections.length > 0
																...
																: ''}
														<span
															class="text-[10px] uppercase tracking-wider rounded border px-1.5 py-0.5 {active ? 'text-error border-error/40' : 'text-cat-gold border-cat-gold/40'}"
